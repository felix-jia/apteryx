...
_memory_usage (void)
{
    ...
    CU_ASSERT (1 == fscanf (f,"%*d %"PRId64"%*d %*d %*d %*d %*d", &memory))
    fclose (f);
    return memory;
}
...
test_lua_load_memory (void)
{
    ...
    L = luaL_newstate ();
    luaL_openlibs (L);
    res = luaL_loadstring (L, "require('apteryx')");
    if (res == 0)
        res = lua_pcall (L, 0, 0, 0);
    if (res != 0)
        fprintf (stderr, "%s\n", lua_tostring(L, -1));
    after = _memory_usage ();
    lua_close (L);
    printf ("%"PRIu64"kb ... ", (after - before) * getpagesize() / 1024);
    CU_ASSERT (res == 0);
}
...
test_lua_load_performance (void)
{
    ...
    for (i = 0; i < TEST_ITERATIONS; i++)
    {
        CU_ASSERT (_run_lua ("require('apteryx')"));
    }
    printf ("%"PRIu64"us ... ", (get_time_us () - start) / TEST_ITERATIONS);
    CU_ASSERT (assert_apteryx_empty ());
}
...
test_lua_load_api_memory (void)
{
    ...
    L = luaL_newstate ();
    luaL_openlibs (L);
    res = luaL_loadstring (L, "require('api')");
    if (res == 0)
        res = lua_pcall (L, 0, 0, 0);
    if (res != 0)
        fprintf (stderr, "%s\n", lua_tostring(L, -1));
...
test_lua_load_api_performance (void)
{
    ...
    for (i = 0; i < 10; i++)
    {
        CU_ASSERT (_run_lua ("require('api')"));
    }
...
test_lua_api_perf_get ()
{
    ...
    L = luaL_newstate ();
    luaL_openlibs (L);
    CU_ASSERT (luaL_loadstring (L, "require('api')") == 0);
    CU_ASSERT(lua_pcall (L, 0, 0, 0) == 0);
    start = get_time_us ();
    for (i = 0; i < TEST_ITERATIONS; i++)
    {
